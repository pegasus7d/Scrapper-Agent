...
Read `DESIGN.md` first for the system contract; this file only holds phase 3's
step-by-step build order and rationale. See `WORKFLOW.md` for the recurring
process this and every phase file follows.

Same workflow rules as `PHASE1.md`/`PHASE2.md`. Two things motivated this phase:
...
mode `PHASE2.md` step 8 just hit for `routes.py`/`repo.py`) — so formalize the
...
4. **GitHub curated question-bank source.** Ingests
   ...
   - The questions relevance-gate prompt (`PHASE2.md` step 2) relaxes the "a

    ...
    <div className="rounded-xl border border-indigo-200 bg-indigo-50/40 p-5">
      ...
      <dl className="mt-4 grid grid-cols-4 gap-4 text-center">
        <div>
          ...
          <dd className="text-lg font-semibold text-slate-900">{run.pages_fetched}</dd>
        </div>
        <div>
          <dt className="text-xs text-muted-foreground">Saved</dt>
          <dd className="text-lg font-semibold text-slate-900">{run.items_saved}</dd>
        </div>
        <div>
          <dt className="text-xs text-muted-foreground">Duplicates</dt>
          <dd className="text-lg font-semibold text-slate-900">{run.items_duplicate}</dd>
        </div>
        <div>
          <dt className="text-xs text-muted-foreground">Errors</dt>
          <dd className="text-lg font-semibold text-slate-900">{run.errors.length}</dd>
        </div>
      </dl>

      {recentErrors.length > 0 && (
        <ul className="mt-4 space-y-1 border-t border-indigo-100 pt-3 text-xs text-slate-500">

...
- Work on exactly **one feature/step at a time** (build order in the current phase's
  `PHASE{N}.md`, indexed from `DESIGN.md` §8). Finish it, validate it
...
(`PHASE4.md`, stop at step 4 — in progress). When a new phase's build order is
written, add its (file, final step) pair here rather than re-deriving the
prompt from scratch.
